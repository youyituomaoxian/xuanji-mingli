...
(function (global) {
  ...
  var FLOAT_README = [
    ...
    '  会自动打开线上网页版（需联网）并把该档案带入，无需重复录入；',
    '  若本目录恰好放有网页工作台文件（index.html），则优先用本地页面。',
    '',
    '【跨端跳转】',
  ...
  var FLOAT_PS1 = [
    ...
    '# 全局错误捕获：任何异常 → 弹窗显示具体错误 + 写入 float-error.log（避免双击后毫无反应）',
    "trap {",
    "  $errMsg = '[' + (Get-Date -Format 'yyyy-MM-dd HH:mm:ss') + '] ' + $_.Exception.Message",
    "  try { $errMsg | Out-File (Join-Path $PSScriptRoot 'float-error.log') -Append -Encoding UTF8 } catch {}",
    "  try { [System.Windows.MessageBox]::Show($errMsg, '玄机命理·悬浮卡错误') | Out-Null } catch {}",
    "  exit 1",
    "}",
    ...
    '        Title="玄机命理·运势悬浮卡" Width="340" Height="500"',
    ...
    '      </Grid.RowDefinitions>',
    '      <Border Grid.Row="0" x:Name="StaleBar" Background="#5C8C2C2C" Padding="10,5" Visibility="Collapsed">',
    '        <TextBlock x:Name="StaleText" Foreground="#F0D9C9C9" FontSize="11" TextWrapping="Wrap" TextAlignment="Center"/>',
    '      </Border>',
    '      <!-- 顶栏：品牌 + 操作（边距统一 16） -->',
    '      <Grid Grid.Row="1" Margin="16,12,16,6">',
    ...
    '          <TextBlock Text="☯" Foreground="#C9A961" FontSize="14" VerticalAlignment="Center" Margin="0,0,6,0"/>',
    '          <TextBlock Text="玄机命理 · 今日运势" Foreground="#E8EDF4" FontSize="13" FontWeight="SemiBold" VerticalAlignment="Center"/>',
    '        </StackPanel>',
    '        <Button x:Name="BtnEdit" Content="档案" Grid.Column="1" Style="{x:Null}" Background="Transparent" Foreground="#C9A961" BorderThickness="0" FontSize="12" Cursor="Hand" Margin="0,0,10,0" Padding="0"/>',
    '        <Button x:Name="BtnMini" Content="—" Grid.Column="2" Background="Transparent" Foreground="#949BA8" BorderThickness="0" FontSize="12" Cursor="Hand" Margin="0,0,10,0" Padding="0"/>',
    '        <Button x:Name="BtnClose" Content="×" Grid.Column="3" Background="Transparent" Foreground="#949BA8" BorderThickness="0" FontSize="13" Cursor="Hand" Padding="0"/>',
    '      </Grid>',
    '      <ScrollViewer Grid.Row="2" VerticalScrollBarVisibility="Auto" Padding="16,4,16,0">',
    '        <StackPanel x:Name="MainPane">',
    '          <!-- 档案编辑表单（紧凑） -->',
    '          <Grid x:Name="EditGrid" Visibility="Collapsed" Margin="0,0,0,10">',
    ...
    '            <Grid.ColumnDefinitions><ColumnDefinition Width="56"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>',
    '            <TextBlock Text="名称" Foreground="#949BA8" FontSize="12" Grid.Row="0" VerticalAlignment="Center"/>',
    '            <TextBox x:Name="InName" Grid.Row="0" Grid.Column="1" Margin="0,3"/>',
    '            <TextBlock Text="性别" Foreground="#949BA8" FontSize="12" Grid.Row="1" VerticalAlignment="Center"/>',
    '            <ComboBox x:Name="InSex" Grid.Row="1" Grid.Column="1" Margin="0,3"><ComboBoxItem Content="男"/><ComboBoxItem Content="女"/></ComboBox>',
    '            <TextBlock Text="年月日" Foreground="#949BA8" FontSize="12" Grid.Row="2" VerticalAlignment="Center"/>',
    '            <TextBox x:Name="InDate" Grid.Row="2" Grid.Column="1" Margin="0,3" ToolTip="格式：1990-5-15（公历）"/>',
    '            <TextBlock Text="时辰" Foreground="#949BA8" FontSize="12" Grid.Row="3" VerticalAlignment="Center"/>',
    '            <TextBox x:Name="InShichen" Grid.Row="3" Grid.Column="1" Margin="0,3" ToolTip="如：午"/>',
    '            <TextBlock Text="地区" Foreground="#949BA8" FontSize="12" Grid.Row="4" VerticalAlignment="Center"/>',
    '            <TextBox x:Name="InPlace" Grid.Row="4" Grid.Column="1" Margin="0,3"/>',
    '            <Button x:Name="BtnSaveCard" Content="保存档案" Grid.Row="5" Grid.Column="1" Background="#2A70B8" Foreground="White" BorderThickness="0" Margin="0,8,0,2" Cursor="Hand" Padding="6,4"/>',
    '          </Grid>',
    '          <!-- 运势主视图（平面设计四原则排版：对比/重复/对齐/亲密性） -->',
    '          <StackPanel x:Name="ViewPane">',
    '            <!-- ① 运势等级：36px 金色居中 = 唯一视觉焦点（强对比） -->',
    '            <StackPanel Orientation="Horizontal" HorizontalAlignment="Center" Margin="0,8,0,0">',
    '              <TextBlock Text="☯" Foreground="#C9A961" FontSize="20" VerticalAlignment="Center" Margin="0,0,8,0"/>',
    '              <TextBlock x:Name="TxtTone" FontSize="36" FontWeight="Bold" Foreground="#C9A961" VerticalAlignment="Center"/>',
    '            </StackPanel>',
    '            <TextBlock x:Name="TxtToneNote" Foreground="#A8B4C2" FontSize="12" TextAlignment="Center" TextWrapping="Wrap" Margin="0,4,0,12"/>',
    '            <!-- 分隔线（重复节奏） -->',
    '            <Rectangle Height="1" Fill="#26C9A961" Margin="6,0,6,12"/>',
    '            <!-- ② 宜/忌 左右两列卡片（亲密成组 + 严格对称对齐） -->',
    '            <Grid Margin="0,0,0,12">',
    '              <Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>',
    '              <Border Background="#17302B" CornerRadius="8" Padding="12,9" Margin="0,0,4,0" BorderBrush="#33C9A961" BorderThickness="1">',
    '                <StackPanel>',
    '                  <TextBlock Text="宜" FontWeight="Bold" Foreground="#C9A961" FontSize="13" Margin="0,0,0,4"/>',
    '                  <TextBlock x:Name="TxtYi" Foreground="#E8EDF4" FontSize="12" TextWrapping="Wrap" LineHeight="18"/>',
    '                </StackPanel>',
    '              </Border>',
    '              <Border Grid.Column="1" Background="#1D2833" CornerRadius="8" Padding="12,9" Margin="4,0,0,0" BorderBrush="#22A8B4C2" BorderThickness="1">',
    '                <StackPanel>',
    '                  <TextBlock Text="忌" FontWeight="Bold" Foreground="#8E99A8" FontSize="13" Margin="0,0,0,4"/>',
    '                  <TextBlock x:Name="TxtJi" Foreground="#C4CCD6" FontSize="12" TextWrapping="Wrap" LineHeight="18"/>',
    '                </StackPanel>',
    '              </Border>',
    '            </Grid>',
    '            <!-- ③ 今日提醒（金色小字居中） -->',
    '            <TextBlock x:Name="TxtWarn" Foreground="#C9A961" FontSize="11" TextAlignment="Center" TextWrapping="Wrap" Margin="2,0,0,12"/>',
    '            <!-- ④ 四柱 2×2 网格（对齐成组） -->',
    '            <Grid Margin="0,0,0,12">',
    '              <Grid.RowDefinitions><RowDefinition/><RowDefinition/></Grid.RowDefinitions>',
    '              <Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="*"/></Grid.ColumnDefinitions>',
    '              <StackPanel Grid.Row="0" Grid.Column="0" Margin="0,0,8,10">',
    '                <TextBlock Text="事业" Foreground="#6B7280" FontSize="10" Margin="0,0,0,3"/>',
    '                <TextBlock x:Name="TxtF1" Foreground="#E8EDF4" FontSize="12" TextWrapping="Wrap" LineHeight="18"/>',
    '              </StackPanel>',
    '              <StackPanel Grid.Row="0" Grid.Column="1" Margin="8,0,0,10">',
    '                <TextBlock Text="财运" Foreground="#6B7280" FontSize="10" Margin="0,0,0,3"/>',
    '                <TextBlock x:Name="TxtF2" Foreground="#E8EDF4" FontSize="12" TextWrapping="Wrap" LineHeight="18"/>',
    '              </StackPanel>',
    '              <StackPanel Grid.Row="1" Grid.Column="0" Margin="0,0,8,0">',
    '                <TextBlock Text="感情" Foreground="#6B7280" FontSize="10" Margin="0,0,0,3"/>',
    '                <TextBlock x:Name="TxtF3" Foreground="#E8EDF4" FontSize="12" TextWrapping="Wrap" LineHeight="18"/>',
    '              </StackPanel>',
    '              <StackPanel Grid.Row="1" Grid.Column="1" Margin="8,0,0,0">',
    '                <TextBlock Text="健康" Foreground="#6B7280" FontSize="10" Margin="0,0,0,3"/>',
    '                <TextBlock x:Name="TxtF4" Foreground="#E8EDF4" FontSize="12" TextWrapping="Wrap" LineHeight="18"/>',
    '              </StackPanel>',
    '            </Grid>',
    '            <!-- ⑤ 幸运信息（居中一行） -->',
    '            <TextBlock x:Name="TxtLucky" Foreground="#A8B4C2" FontSize="11" Margin="2,0,0,12" TextAlignment="Center"/>',
    '            <!-- ⑥ 每日一言（引用卡） -->',
    '            <Border Background="#22242E" CornerRadius="8" Padding="12,10" Margin="0,0,0,12" BorderBrush="#22C9A961" BorderThickness="1">',
    '              <TextBlock x:Name="TxtQuote" Foreground="#C9D4DC" FontSize="11.5" TextWrapping="Wrap" TextAlignment="Center" FontStyle="Italic"/>',
    '            </Border>',
    '            <!-- ⑦ 三派天机简析（次要信息弱化） -->',
    '            <TextBlock Text="── 三派天机简析 ──" Foreground="#5A6572" FontSize="10" TextAlignment="Center" Margin="0,0,0,6"/>',
    '            <TextBlock x:Name="TxtSchools" Foreground="#8E99A8" FontSize="11" TextWrapping="Wrap" LineHeight="18" Margin="2,0,0,4"/>',
    '          </StackPanel>',
    '        </StackPanel>',
    '      </ScrollViewer>',
    '      <!-- 底栏：开机自启 + 查看完整命盘（分隔线 + 统一边距） -->',
    '      <Border Grid.Row="3" BorderBrush="#26C9A961" BorderThickness="0,1,0,0" Padding="16,10,16,12" Background="#0E1A22">',
    '        <Grid>',
    '          <Grid.ColumnDefinitions><ColumnDefinition Width="*"/><ColumnDefinition Width="Auto"/></Grid.ColumnDefinitions>',
    '          <CheckBox x:Name="ChkAuto" Content="开机自启" Foreground="#949BA8" FontSize="11" VerticalAlignment="Center"/>',
    '          <Button x:Name="BtnOpen" Content="查看完整命盘 →" Grid.Column="1" Background="#2A70B8" Foreground="White" BorderThickness="0" Padding="12,6" Cursor="Hand" FontSize="12"/>',
    '        </Grid>',
    '      </Border>',
    '    </Grid>',
    '  </Border>',
    ...
    '$txtTone = Find "TxtTone"; $txtToneNote = Find "TxtToneNote"',
    '$txtF1 = Find "TxtF1"; $txtF2 = Find "TxtF2"; $txtF3 = Find "TxtF3"; $txtF4 = Find "TxtF4"',
    ...
    '  $txtF1.Text = ""; $txtF2.Text = ""; $txtF3.Text = ""; $txtF4.Text = ""',
    '  $txtYi.Text = ""; $txtJi.Text = ""; $txtWarn.Text = ""',
    ...
    '  $f4 = @($s.daily.four)',
    '  if ($f4.Count -ge 1) { $txtF1.Text = [string]$f4[0].v }; if ($f4.Count -ge 2) { $txtF2.Text = [string]$f4[1].v }',
    '  if ($f4.Count -ge 3) { $txtF3.Text = [string]$f4[2].v }; if ($f4.Count -ge 4) { $txtF4.Text = [string]$f4[3].v }',
    ...
    '  # 查看完整命盘：优先打开线上工作台（普通用户只有 zip，无本地 index.html）；',
    '  # 若同目录恰好有 index.html（开发者/本机离线版），则用本地页面。',
    '  # 档案来源优先级：快照 profile（导出时自带的命盘）> 本地档案卡（float-cards.json）',
    '  $webUrl = "https://xuanji-mingli.pages.dev/"',
    '  $c = $null',
    '  if ($script:data -and $script:data.snapshot -and $script:data.snapshot.profile) {',
    '    $p = $script:data.snapshot.profile',
    '    $dp = [string]$p.solar -split "-"',
    '    if ($dp.Count -ge 3) {',
    '      $c = [pscustomobject]@{',
    '        name = $p.name; sex = if ($p.sex -eq "女") { "female" } else { "male" };',
    '        cal = "solar"; year = [int]$dp[0]; month = [int]$dp[1]; day = [int]$dp[2];',
    '        shichen = $p.shichen; place = $p.place',
    '      }',
    '    }',
    '  }',
    '  if (-not $c -and $script:cards -and $script:cards.cards.Count -gt 0) { $i = [Math]::Min($script:cards.current, $script:cards.cards.Count-1); $c = $script:cards.cards[$i] }',
    '  $q = "auto=1"',
    '  if ($c) {',
  ...
  /* 悬浮卡启动器（.bat 纯 ASCII，双击即运行，自动绕过 ExecutionPolicy）
     注：-WindowStyle Hidden 隐藏控制台；PS1 内部 trap 会在出错时弹窗提示，
     不再出现「双击后毫无反应」的体验。 */
  var FLOAT_BAT = [
    '@echo off',
